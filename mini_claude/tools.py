...
PermissionMode = str  # 可选值："default" | "plan" | "acceptEdits" | "bypassPermissions" | "dontAsk" | "auto"
...
def _load_settings(file_path: Path) -> dict | None:
    """安全读取 JSON 设置文件；文件不存在或无效时返回 None。"""
...
def check_permission(
    ...
    """返回包含 `action` 和可选 `message` 的权限判定结果。"""
    # deny 规则始终优先。即使使用 bypassPermissions（--yolo），
    # 也仍受 deny 规则约束（见 docs/06-permissions.md），
    # 因此必须在任何模式快捷判断之前检查它们。
    rule_result = _check_permission_rules(tool_name, inp)
    if rule_result == "deny":
        return {"action": "deny", "message": f"Denied by permission rule for {tool_name}"}

    # plan 模式的只读约束优先于 allow 规则和绕过权限设置：
    # 只允许写入计划文件本身，shell 仍被禁止（见 docs/10）。
    if mode == "plan":
